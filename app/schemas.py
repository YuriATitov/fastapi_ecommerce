...
class Product(BaseModel):
    ...
    updated_at: datetime = Field(..., description= "Дата и время обновления записи")
...
class ReviewCreate(BaseModel):
    """
    Модель для создания и обновления отзыва о товаре.
    Используется в POST и PUT запросах.
    """
    product_id: int = Field(..., description= "ID продукта, на который оставляем отзыв")
    comment: str = Field(..., description= "Отзыв о товаре")
    grade: int = Field(..., gt=0, le=5, description="Оценка товара от 1 до 5")
...
class Review(BaseModel): 
    ...
    user_id: int = Field(..., description= "ID клиента оставившего отзыв")
    product_id: int = Field(..., description= "ID продукта, на который оставляем отзыв")
    comment: str = Field(..., description= "Отзыв о товаре")
...
class OrderList(BaseModel):
    ...
    total: int = Field(ge=0, description="Общее кол-во заказов")
